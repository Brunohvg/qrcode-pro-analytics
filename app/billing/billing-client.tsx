...
const planInfo = {
  Pro: { price: "49,90", label: "Pro", description: "Campanhas, UTM, relatórios, senha, validade e branding." },
  Business: { price: "119,90", label: "Business", description: "Lote, Smart Redirect, domínio próprio, integrações, API e webhooks." },
} as const;
...
export default function BillingClient({
  ...
  return <main className="shell py-10 md:py-14">
    <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
      <div><span className="badge">Assinatura e pagamentos</span><h1 className="text-3xl md:text-4xl font-black mt-3">Gerencie seu plano</h1><p className="muted mt-2">Mercado Pago para recorrência automática ou PIX como alternativa.</p></div>
    ...
    {loading && !data ? <div className="card p-8 muted">Carregando...</div> : data && <>
      <section className="card p-6 mb-7 flex flex-wrap items-center justify-between gap-5">
        ...
        {data.subscription.plan.name !== "Gratuito" && <button onClick={()=>void cancel()} disabled={action === "cancel"} className="btn-secondary !text-rose-300">{action === "cancel" ? "Cancelando..." : "Cancelar assinatura"}</button>}
      </section>
      ...
      <section className="grid lg:grid-cols-2 gap-5 mb-7">
        {(Object.keys(planInfo) as Array<keyof typeof planInfo>).map((name) => {
          ...
          return <article key={name} className={`card p-7 ${highlighted ? "ring-2 ring-emerald-300/70" : ""}`}>
            <h2 className="text-2xl font-black">{plan.label}</h2>
            <div className="mt-3"><span className="text-4xl font-black">R$ {plan.price}</span><span className="muted">/mês</span></div>
            <p className="muted mt-3">{plan.description}</p>
            <div className="grid sm:grid-cols-2 gap-2 mt-6">
              <button disabled={!data.paymentOptions.mercadoPago || !!action} className="btn-primary" onClick={()=>void checkout(name)}>
                {action === `mp-${name}` ? "Abrindo..." : "Assinar com Mercado Pago"}
              </button>
              <button disabled={!data.paymentOptions.manualPix || !!action} className="btn-secondary" onClick={()=>void manualPix(name)}>
      ...
      {pix && <section className="card p-6 mb-7 border-emerald-300/30">
        ...
        <p className="muted text-sm mt-2">Após pagar, a confirmação será feita no painel administrativo. Seu plano é liberado sem alterar QR Codes ou métricas existentes.</p>
        <div className="rounded-xl bg-black/20 border border-white/10 p-4 mt-5">
          ...
          <button onClick={()=>copy(pix.pix.key)} className="btn-secondary !py-2 mt-3">Copiar chave</button>
          {pix.pix.recipientName && <div className="muted text-sm mt-3">Recebedor: {pix.pix.recipientName}{pix.pix.city ? ` · ${pix.pix.city}` : ""}</div>}
          <div className="muted text-xs mt-3">Referência: {pix.payment.externalReference}</div>
        </div>
