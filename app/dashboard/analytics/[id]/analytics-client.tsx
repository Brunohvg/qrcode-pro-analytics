...
type Analytics = {
  ...
  plan: { name: string; analyticsDays: number };
  summary: { last24: number; period: number; lifetime: number };
...
export default function AnalyticsClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const response = await fetch(`/api/analytics/${id}`, { cache: "no-store" });
        const body = await response.json();
        if (!response.ok) throw new Error(body.message || "Falha ao carregar analytics.");
        setData(body);
      } catch (err) {
  ...
  const periodLabel = data.plan.analyticsDays < 0 ? "Histórico disponível" : `Últimos ${data.plan.analyticsDays} dias`;

  return <main className="shell py-10 md:py-14">
    <div className="flex flex-wrap justify-between gap-4 items-start mb-7"><div><Link href="/dashboard" className="muted text-sm">← Dashboard</Link><h1 className="text-3xl font-black mt-2">{data.qr.name}</h1><p className="muted mt-1 truncate max-w-2xl">{data.qr.originalUrl}</p></div><span className="badge">{data.plan.name} • {data.plan.analyticsDays < 0 ? "histórico completo" : `${data.plan.analyticsDays} dias`}</span></div>
    <div className="grid md:grid-cols-3 gap-4 mb-5">{[["Últimas 24h",data.summary.last24],[periodLabel,data.summary.period],["Total histórico",data.summary.lifetime]].map(([label,value])=><div className="card p-5" key={String(label)}><div className="muted text-sm">{label}</div><div className="text-3xl font-black mt-2">{value}</div></div>)}</div>
    <div className="grid lg:grid-cols-[1.5fr_.8fr] gap-5">
      <section className="card p-6"><h2 className="font-black text-lg">Acessos por dia</h2><div className="mt-6 h-56 flex items-end gap-1.5 border-b border-white/10">{data.byDay.length===0?<p className="muted m-auto">Sem scans no período disponível.</p>:data.byDay.map((day)=><div key={day.label} className="group flex-1 min-w-1 bg-emerald-300/70 hover:bg-emerald-200 rounded-t relative" style={{height:`${Math.max(4,(day.count/maxDay)*100)}%`}} title={`${day.label}: ${day.count}`} />)}</div></section>
